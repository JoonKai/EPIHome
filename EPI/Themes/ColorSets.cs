...
namespace EPI.Themes
{
    public class ColorSets
    {
        // 현재 활성 팔레트 (외부에서 교체 가능)
        private List<Color> _colorSet = null;

        /// <summary>
        /// 현재 사용 중인 ColorSet (null이면 기본 팔레트 사용)
        /// </summary>
        public IList<Color> ColorSet => _colorSet ?? GetColors().ToList();
        ...
        /// <summary>
        /// 외부(EPIWaferMapRange 등)에서 팔레트 교체 시 사용
        /// </summary>
        public void SetColors(IList<Color> colors)
        {
            if (colors == null || colors.Count == 0) return;
            _colorSet = new List<Color>(colors);
        }

        /// <summary>
        /// 값을 레인지에 매핑하여 대응 색상 반환
        /// rangeStart 이하 → 첫 번째 색, rangeEnd 이상 → 마지막 색
        /// </summary>
        public Color ValueToColor(double rangeStart, double rangeEnd, double value)
        {
            var palette = ColorSet;
            int count = palette.Count;

            if (count == 0) return Colors.Transparent;
            if (rangeEnd <= rangeStart) return palette[0];

            // 0.0 ~ 1.0 으로 정규화
            double t = (value - rangeStart) / (rangeEnd - rangeStart);
